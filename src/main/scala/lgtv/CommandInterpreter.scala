...
trait CommandInterpreter[F[_]] {
  def interpret(command: String): F[Unit]
}

object CommandInterpreter {
  def apply(tv: LGTV[IO])(implicit timer: Timer[IO]): CommandInterpreter[IO] = (command: String) => {

    def _interpret(command: String): IO[Either[Command, List[Command]]] = command match {
      //single commands
    ...
    command.trim.toLowerCase match {
      case "on" => tv.wake()
      case com => _interpret(com).flatMap {
